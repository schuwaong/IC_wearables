...
function promptWithReferenceLock(prompt, referenceImages = []) {
  ...
  return [
    cleanPrompt,
    "Hard reference lock: Image 1 is the only identity source and the only hair source. The generated person must be the exact same person from Image 1, not a similar model, not a beautified version, and not a catalogue model. Preserve the exact facial identity, head shape, head width, forehead size, facial proportions, jawline, cheekbones, eyes, brows, nose, lips, ears, skin tone, facial asymmetry, natural skin texture, hairstyle, hairline, baby hairs, hair length, hair volume, hair density, hair part, bangs or fringe, flyaways, hair colour, facial hair, and current expression. If the reference is neutral or unsmiling, keep it neutral. Do not add a smile or visible teeth unless already present in Image 1. Keep the same face scale, same crop logic, same head-to-body relationship, and same hairstyle silhouette around the forehead, temples, ears, and shoulders.",
    "Face-and-hair protection rule: do not beautify, retouch, warp, liquify, stretch, slim, widen, sharpen into a new face, age-shift, face-swap, hair-swap, restyle the hair, add volume, remove volume, change the hairline, change the part, tuck hair differently, shorten hair, lengthen hair, recolour hair, smooth skin excessively, or replace the face. The hairstyle must survive the clothing change unchanged.",
    ...
    "Conflict rule: when styling instructions conflict with identity preservation, identity preservation wins. It is better to simplify the outfit than to alter the face or hairstyle.",
  ].join(" ");
}
